...
const components: JSXMapSerializer = {
  ...
  paragraph: ({ children }) => (
    <p
      className="text-xl md:text-2xl font-normal font-body text-slate-600 mb-8"
    >
      {children}
    </p>
  )
...
const Testimonials = async ({ slice }: TestimonialsProps): Promise<JSX.Element> => {
  ...
  const testimonials = await Promise.all(
    slice.primary.testimonials.map((testimonial) => {
      if (isFilled.contentRelationship(testimonial.testimonial) && testimonial.testimonial.uid) {
